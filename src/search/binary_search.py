...
def binary_search_inter(l:list, start:int, end:int, target:int, hook_func=None) -> int:
    '''
    @brief  插值查找法
    @param  l   目标数组，必须有序
    @param  start   起始位置，指向元素的起始位置，一般为0
    ...
    while (start < max_index):
        if l[max_index] == l[left]:
            return -1
            
        mid = start + (max_index - start) * (target - l[start]) / (l[max_index] - l[start])
        mid = int(mid)
        if None != hook_func:
            hook_func(data=l, start=start, mid=mid, end=end, target=target, count=count)
            count += 1
        
        if mid < min_index or mid > max_index:
            return -1
